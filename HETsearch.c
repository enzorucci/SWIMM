					...
					for (t=0; t< cpu_vect_sequences_db_count; t++) {
						...
						for (i=0; i< ptr_chunk_n[s] ;i++ ) {
							...
							for (j=0; j< BLOSUM_ROWS-1; j++) {
								...
								_mm_store_si128(ptr_scoreProfile1+j*ptr_chunk_n[s],   aux7);
							}
							_mm_store_si128(ptr_scoreProfile1+(BLOSUM_ROWS-1)*ptr_chunk_n[s],  vzero_epi8);
						}
					...
					for (t=0; t< cpu_vect_sequences_db_count; t++) {
						...
						for (i=0; i< ptr_chunk_n[s] ;i++ ) {
							...
							for (j=0; j< BLOSUM_ROWS-1; j++) {
								...
								_mm256_store_si256(ptr_scoreProfile1+j*ptr_chunk_n[s],_mm256_or_si256(aux5,aux6));
							}
							_mm256_store_si256(ptr_scoreProfile1+(BLOSUM_ROWS-1)*ptr_chunk_n[s],vzero_epi8);
						}
